use crate::{models::user::User, utils::state::AppState};
use axum::{
    extract::{Path, State},
    http::StatusCode,
...
pub async fn create_user(
    ...
    let body = json!({"name": payload.name, "username": payload.username, "email": payload.email, "dob": payload.dob, "password": payload.password, "auth_provider":payload.auth_provider});
    let response = state
        .supabase
        .from("Users")
